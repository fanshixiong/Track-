...
def show_precision(positions, ground_truth):
	...
	for i in range(length + 1):
		...
		target[i - 1, 0] = int(target[i-1, 0]*5/8)
		target[i-1, 1] = ground_truth[i - 1, 1] + 0.5*ground_truth[i-1, 3]
		target[i - 1, 1] = int(target[i - 1, 1] * 2 / 3)
	...
	np.savetxt("pos.txt", pos, fmt='%f', delimiter=',')
	np.savetxt("target.txt", target, fmt='%f', delimiter=',')
