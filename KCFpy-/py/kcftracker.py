...
class KCFTracker:
	def __init__(self, hog=False, fixed_window=True, multiscale=False):
		...
		self._reg = [0., 0., 0., 0.]#监控区域
		self._enter = [0., 0., 0., 0.]#入口检测框
		self._exact = 0 #是否检测到目标
	...
	def gaussianCorrelation(self, x1, x2):
		if (self._hogfeatures):
			...
			c = cv2.mulSpectrums(fftd(x1), fftd(x2), 0, conjB=True)   # 'conjB=' is necessary! x1是fitures x2是template
			c = fftd(c, True)
			c = real(c)
			c = rearrange(c)
	...
	def setArea(self, region):
		self._reg[0] = region[0]
		self._reg[1] = region[1]
		self._reg[2] = region[2]
		self._reg[3] = region[3]


	def monitoring(self):
		if (self._roi[0]>=self._reg[0] and self._roi[1]>=self._reg[1] and
			self._roi[0]+self._roi[2]<=self._reg[0]+self._reg[2] and self._roi[1]+self._roi[3]<=self._reg[1]+self._reg[3]):
			return 1
		return 0


	def setEntrance(self, entrance):
		self._enter[0] = entrance[0]
		self._enter[1] = entrance[1]
		self._enter[2] = entrance[2]
		self._enter[3] = entrance[3]


	def checkIfEnter(self, image):
		self._roi = list(map(float, self._enter))
		assert (self._enter[2] > 0 and self._enter[3] > 0)
		#self._tmpl = self.getFeatures(image, 1)
		self._prob = self.createGaussianPeak(self.size_patch[0], self.size_patch[1])
		#self._alphaf = np.zeros((self.size_patch[0], self.size_patch[1], 2), np.float32)
		#self.train(self._tmpl, 1.0)
		loc, peak_value = self.detect(self._tmpl, self.getFeatures(image, 1, 1.0))
		if (peak_value >= 0.36 and peak_value <= 0.4):
			self._exact = 1

		else:
			self._exact = 0
		return self._exact, peak_value, self._roi
